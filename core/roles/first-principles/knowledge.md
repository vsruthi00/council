...
**Principle:** Decompose a problem into its most basic, verifiable truths. Refuse to accept inherited solutions as starting points. Build up only from what can be directly confirmed.
...
**Principle:** Watch for these recurring false constraints: (1) "Our users expect X" stated without data. (2) "The current stack requires Y" where the requirement is actually a sunk-cost preference. (3) "Regulations prohibit Z" stated without citing the specific rule. (4) "We tried this before and it failed" without specifying which conditions caused failure. Each must be verified before being accepted.
...
**Principle:** Force each claim to its foundation by asking: What does this rest on? How do we know that? What would have to be false for this to fail? Keep asking until either a bedrock fact is reached or the claim collapses. Record which claims could not be grounded.
