...
## Beginner's mind questioning

**Principle:** Approach the problem as if you have no prior investment in the existing approach. A beginner does not know "the way things are done here" and therefore asks the questions that insiders stopped asking years ago. The most valuable beginner questions are usually the ones that produce an uncomfortable silence, not the ones that get answered easily.
...
**Principle:** Problems that appear novel in one field have often been solved in a different field. Importing the solution from the other field either validates the current approach ("this is what aviation did and it works") or reveals a gap ("aviation solved this by doing X; the proposal does not include X"). The analogy must be concrete and the mapping must be stated explicitly.
...
**Principle:** If you cannot explain a proposal clearly to someone who has never seen the domain, the proposal likely contains hidden complexity that the team has not resolved. The explanation exercise forces the team to surface those complexities. The outsider uses this as a diagnostic: wherever the explanation breaks down is where the proposal has a gap.
